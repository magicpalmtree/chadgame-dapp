<template>
    <div class="flex relative items-center justify-center">
        ...
        <a 
            class="flex flex-row rounded-full absolute text-xl bg-white px-6 py-1 font-bold text-secondary bottom-4 left-1/2 -translate-x-1/2 bg-white hover:cursor-pointer hover:text-secondary-light"
            :href="visitUrl ? visitUrl : ''"
            :target="visitUrl && visitUrl.startsWith('#') ? '_self' : '_blank'"
        >
            <div class="btn-text">
                {{btnText}}
            </div>
